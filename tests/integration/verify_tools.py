...
def test_tools():
    ...
    try:
        ...
        print(f"✓ Latest energy_futures: {result['value']} on {result['date']}")
    except Exception as e:
        print(f"✗ Error: {e}")
    
    # Test 2: Historical data by date
    ...
    try:
        ...
        print(f"✓ cotton_price on 2024-01-01: {result['value']}")
    except Exception as e:
        print(f"✗ Error: {e}")
    ...
    # Summary
    print("\n" + "=" * 60)
    print(f"\n✓ All {len(TOOLS)} tools registered successfully!")
    print("\nRegistered tools:")
    for tool in TOOLS:
